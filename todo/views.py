...
from .forms import LoginForm, RegisterForm, TasklistForm, TaskForm, ProfileForm
...
def loginuser(request):
        if request.method == "GET":
            ...
            if user is None:
                return render(request, 'todo/login.html', {'form':LoginForm(), 'error':'Identifiants invalides.'})
            else:
                login(request, user)
                return redirect('currenttasks')
